...
void calcBoundary(double* u, double* v, double* o, int thrn, int thread_n)
{
	for (int y = 1; y < Y - 1; y++) {
		for (int x = thrn + 1; x < X - 1; x += thread_n) {
			...
			GET(u, x, y, Z - 1) = fmax((4 * GET(u, x, y, Z - 2) - GET(u, x, y, Z - 3)) / 3, 0);
			GET(v, x, y, Z - 1) = fmax((4 * GET(v, x, y, Z - 2) - GET(v, x, y, Z - 3)) / 3, 0);
			GET(o, x, y, Z - 1) = getNextO(u, o, o0, x, y, Z - 1);
		}
		for (int z = thrn + 1; z < Z - 1; z += thread_n) {
